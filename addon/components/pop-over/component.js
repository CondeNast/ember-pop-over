...
import { later, once, next, scheduleOnce, bind } from "@ember/runloop";
...
export default Component.extend({
  ...
  mouseLeave() {
    ...
    later(() => {
      ...
      if (
        A(A(get(this, "targets")).filterBy("_willLeave", false)).isAny(
          "hovered"
        )
      ) {
        this._willLeave = false;
  ...
  documentClick(evt) {
    ...
    var clicked =
      evt.target === element || jQuery.contains(element, evt.target);
  ...
  activeTarget: computed("activeTargets.[]", {
    get() {
      if (get(this, "areAnyTargetsActive")) {
        return (
          get(this, "targets").findBy("anchor", true) ||
  ...
  /**
    ...
    menu.
   */
  visibilityDidChange: on(
    "init",
    observer("areAnyTargetsActive", function() {
      once(() => {
        var proxy = (this.__documentClick =
          this.__documentClick || bind(this, "documentClick"));
  ...
  tile() {
    ...
    if (get(this, "supportsLiquidFire")) {
      $popover = $element.find(
        "> .liquid-container > .liquid-child > .pop-over-compass"
      );
    ...
    if (gravityName) {
      ...
      assert(
        `There is no gravity "${gravityName}".
         ...
         Please choose one of ${Object.keys(gravity).map(dir => `"${dir}"`)}.`,
        constraints
      );
    } else {
    ...
    for (let i = 0, len = constraints.length; i < len; i++) {
      solution = constraints[i].solveFor(
        boundingRect,
        targetRect,
        popOverRect,
        pointerRect,
    ...
    set(
      this,
      "hidden",
      Rectangle.intersection(boundingRect, targetRect).area === 0
    );
